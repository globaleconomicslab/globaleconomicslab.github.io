// Site-wide soft lock screen. This is a client-side deterrent only (the
// password is visible in this file's source), not real access control —
// it just keeps the site from being casually stumbled on before launch.
// Change PASSWORD below to update it. Unlocking is remembered for the
// browser session (sessionStorage), so it re-prompts on a new session.
(function () {
  var PASSWORD = 'gel2026';
  var STORAGE_KEY = 'gel_unlocked';
  ...
  function reveal() {
    ...
    var gate = document.getElementById('site-lock-gate');
    if (gate) gate.parentNode.removeChild(gate);
  }

  function showGate() {
    var gate = document.createElement('div');
    gate.id = 'site-lock-gate';
    gate.innerHTML =
      '<div class="site-lock-box">' +
        '<span class="site-lock-mark">Global Economics Lab</span>' +
        '<h2>This site is currently private</h2>' +
        '<p>Enter the password to continue.</p>' +
        '<form id="site-lock-form" autocomplete="off">' +
          '<input type="password" id="site-lock-input" placeholder="Password" autocomplete="new-password" required>' +
          '<button type="submit" class="btn btn-primary">Enter</button>' +
        '</form>' +
        '<p class="site-lock-error" id="site-lock-error" hidden>Incorrect password. Try again.</p>' +
      '</div>';
  ...
  if (bioModal) {
    var bios = {
      gita: {
        ...
        bio: 'Gita Gopinath is the Gregory and Ania Coffey Professor of Economics in the Department of Economics at Harvard University, and Professor of Public Policy at Harvard Kennedy School. She is the Founding Director of the Global Economics Lab. Her research focuses on International Finance and Macroeconomics where she is a leading voice on dollar dominance, exchange rates, trade and investment, international financial crises, monetary policy and debt. Previously, she was the First Deputy Managing Director (FDMD) of the International Monetary Fund (IMF) from January 2022–August 2025 and prior to that she was the Chief Economist of the IMF from January 2019-January 2022.',
        links: [
