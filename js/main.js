...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Homepage featured-projects hero: two tiles (primary + secondary)
  // cycling through 3 pages drawn from a pool of 6 projects, with a dot
  // bar overlaid on each image (so it appears twice) to jump between
  // pages directly. Auto-advances every 20s (first advance sooner, at
  // 8s, so it doesn't feel static on load) and pauses on hover. Both
  // tiles share one background image — only the title, subtitle, and
  // link swap; the photo itself never changes. The swap reads as turning
  // a page: each tile's text slides out to the left and fades, the new
  // text is placed off-screen right, then slides in to settle — rather
  // than a plain crossfade in place.
  var projectGrid = document.getElementById('project-grid');
  var projectDotEls = projectGrid ? Array.prototype.slice.call(projectGrid.querySelectorAll('.research-hero-dot')) : [];
  if (projectGrid && projectDotEls.length) {
    var projects = [
      { title: 'How to Address Global Imbalances?', subtitle: 'The G7 economists’ report proposed solutions that were core to the discussions of the G7 French Presidency in 2026.', href: 'g7-global-imbalances.html' },
      { title: 'The Future of Tax Policy: Public Finance in the Age of AI', href: 'public-finance-age-of-ai.html' },
      { title: 'Stablecoins and Anonymous Money', href: 'stablecoins-anonymous-money.html' },
      { title: 'U.S.–China Trade War', href: 'remains-of-the-trade.html' },
      { title: 'Economic Policy Challenges for the Age of AI', href: 'economic-policy-age-of-ai.html' },
      { title: 'What Happens to Monetary Policy Without Commitment', href: 'monetary-policy-without-commitment.html' }
    ];
    var projectPages = [[0, 1], [2, 3], [4, 5]];
    var projectSlots = [0, 1].map(function (i) { return document.getElementById('project-slot-' + i); });
    var currentPage = 0;
    var projectTimer = null;
    var PROJECT_LEAVE_MS = 800;
    var PROJECT_INITIAL_DELAY_MS = 8000;
    var PROJECT_INTERVAL_MS = 20000;

    function renderPage(pageIndex) {
      projectPages[pageIndex].forEach(function (projIdx, slotIdx) {
        var slot = projectSlots[slotIdx];
        if (!slot) return;
        var p = projects[projIdx];
        var heading = slot.querySelector('h2, h3');
        var sub = slot.querySelector('.research-hero-subtitle');
    ...
    function goToPage(pageIndex) {
      ...
      projectSlots.forEach(function (slot) { if (slot) slot.classList.add('is-leaving'); });
      setTimeout(function () {
        ...
        projectSlots.forEach(function (slot) { if (slot) slot.classList.remove('is-entering'); });
      }, PROJECT_LEAVE_MS);
    }

    function nextPage() { goToPage((currentPage + 1) % projectPages.length); }
