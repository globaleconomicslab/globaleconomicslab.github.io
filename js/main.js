...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Homepage featured-projects grid: 4 visible tiles rotating through a
  // pool of 6 projects every 10s — each cycle draws a fresh random 4 of
  // the 6 (a shuffle, so the same 4 can occasionally repeat). Pauses on
  // hover. All four tiles share one background image (divided into
  // quadrants by the grid, same technique as the old two-tile split
  // hero) — only the title, subtitle, and link swap; the photo itself
  // never changes.
  ...
  if (projectGrid) {
    ...
    var projectSlots = [0, 1, 2, 3].map(function (i) { return document.getElementById('project-slot-' + i); });
    ...
    function renderProjectSet(indices) {
      indices.forEach(function (projIdx, slotIdx) {
    ...
    function swapProjectSet() {
      projectGrid.classList.add('is-swapping');
      setTimeout(function () {
        renderProjectSet(pickRandomFour());
        projectGrid.classList.remove('is-swapping');
      }, 350);
    }

    function startProjectAuto() {
      // First swap comes sooner (4s) so the grid doesn't feel static on
      // load; subsequent swaps settle into the full 10s interval.
      projectTimer = setTimeout(function () {
        swapProjectSet();
        projectTimer = setInterval(swapProjectSet, 10000);
      }, 4000);
    }
